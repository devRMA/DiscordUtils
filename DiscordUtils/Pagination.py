...
class CustomEmbedPaginator(object):
    ...
    async def run(self, embeds, send_to=None, reply=False, mention_author=False):
        ...
        def check(r, u):
            return u == wait_for and r.message.id == msg.id and (str(r.emoji) in self.control_emojis or
        ...
        while True:
            if self.timeout > 0:
                ...
                reaction, user = await self.bot.wait_for("reaction_add", check=check)
            for emoji in self.control_emojis:
                if emoji == str(reaction.emoji) or emoji == reaction.emoji:
                    index = self.control_emojis.index(emoji)
                    ...
                    if cmd.lower() == "first":
                        ...
                        for reaction in msg.reactions:
                            if reaction.me:
                                await reaction.remove(self.ctx.me)
                        return msg
                    elif cmd.startswith("page"):
                        shit = cmd.split()
                        pg = int(shit[1])
                        if self.current_page != pg:
                        ...
                        if something.isdigit():
                            ...
                            emoji = list(filter(lambda x: x == something or str(x) == something,
                                                self.control_emojis))
                            if len(emoji) > 0:
                                emoji = emoji[0]
                                if self.remove_reactions:
